      ...
      {meetings.length === 0 ? (
        ...
        <ul className="space-y-4">
          {meetings.map((meeting: any) => {
            const shortSummary =
              meeting.summary?.split('\n')[0].slice(0, 300).replace(/\.$/, '') + '.' || ''

            return (
              <li key={meeting.id} className="border rounded p-4 hover:bg-gray-50">
                <a href={`/meeting/${meeting.id}`} className="text-blue-600 hover:underline text-lg font-medium">
                ...
                {shortSummary && (
                  <p className="text-gray-700 mt-2 italic">{shortSummary}</p>
                )}
              </li>
            )
          })}
